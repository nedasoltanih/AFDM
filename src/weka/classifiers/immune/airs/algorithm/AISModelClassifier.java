...
public abstract class AISModelClassifier implements Serializable 
{
					...
					boolean Negative_sel)
	{
		...
		if(user_spec)
		{
			LinkedList<Cell> UserSpecCells = new LinkedList<Cell>();		
		
			for(Cell c : affinitySortedCells)
			{
				if((c.getClassification() == 0 && c.getUser() == aInstance.value(0)) 
						|| (c.getClassification() == 1))
					{
						UserSpecCells.add(c);					
					}
			}
			...
			{
				int classIndex = (int) UserSpecCells.get(i).getClassification();
	...
	//edited by neda
	public int classifyInstance(Instance aInstance, int dist, boolean update, boolean user_spec, boolean neg_sel)
	{
		// normalise vector
		try {
		...
		// classify
		//edited by neda
		return classify(aInstance, dist, update, user_spec, neg_sel);
	}	

	//edited by neda
	protected abstract int classify(Instance aInstance,int dist, boolean update, boolean user_spec, boolean neg_sel);
